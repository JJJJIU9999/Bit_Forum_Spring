...
| 真实调用验证 | 已通过：工具调用（M14）；RAG 检索 + 回答带引用（M15-4）；异步索引端到端（M15-5） |
| 待办 | 按交付策略，M15 完成是第一个 push 检查点，**需用户确认后 push** |
