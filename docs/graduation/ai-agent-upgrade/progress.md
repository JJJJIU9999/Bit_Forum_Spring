...
| 测试状态 | **394 项：385 通过 + 9 项条件跳过**（跳过项均为需要真实 API Key 的调用测试）；前端 **36 项** |
...
| M18 | 可观测性、评估与工程化闭环 | **已完成** | T12/T13 前置验证通过；V18 轨迹 + V19 用量 + 统一降级 + 前端轨迹页全部落地；后端 394 项 / 前端 36 项全绿；真实环境端到端验证（一次对话产生 5 步轨迹，含工具调用） | `m18-decision-brief.md`、`m18-handoff.md` |
