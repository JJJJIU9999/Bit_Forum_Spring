...
- **状态：完成。** 读取交接任务书、README、AI 升级文档、导出脚本、Git 状态和 AI 相关测试清单；未读取 `.env`、未发起真实模型调用、未修改生产代码。
- **基线：** `feat/ai-agent` / `514f301`，相对 `main` 领先 41 个提交，`origin/HEAD...HEAD = 0 41`，开始时工作区干净。
- **发现：** 长期文档中混有日期未标注的 Git/推送/测试快照；导出脚本的动态页眉会与这些嵌入快照并列，须在后续文档审计中消除“当前状态”歧义。
- **下一步：** 逐项核验 AI 调用、轨迹、用量和降级测试；先复用现有测试覆盖，再决定是否需要最小测试专用补充。

### Final-1：隔离测试环境修正

- 首次新增测试启动失败：`src/test/resources/application.yml` 的 RabbitMQ 默认 `guest` 凭据不匹配本机容器；未读取 `.env`，且数据库/Flyway 已成功连接与校验。
- 处理：两个不依赖消息消费的测试类仅在其测试上下文关闭 Rabbit listener 自动启动。此设置不影响 `application.yml`、容器或生产运行，避免为了跑测试读取本地凭据。
- 故障注入确认不可达回环端口会触发 Spring AI 默认 10 次重试；为使测试可重复，故障注入类额外限定本测试上下文 `spring.ai.retry.max-attempts=1`，随后重跑。

### Final-2：模型不可用故障注入与最小修复

- 注入方法：测试上下文正常启动后，将 DeepSeek base URL 指向不可达 `127.0.0.1:9`，使用占位 API key，且只重试一次；不访问外网或真实凭据。
- 发现：QA 返回给用户的内容已统一，但 `TraceRecorder` 曾把底层连接详情保存到管理端可见的 Trace message。
- 修复：Trace 只持久化 `TraceDegradeReason` 的统一文案；内部 detail 不再入库，改为 debug 日志。扩展轨迹集成测试防止连接地址再次进入步骤明细。
- 运行原有 Trace 集成测试时再次暴露本机 RabbitMQ 凭据与测试默认值不一致；该类不消费消息，按新增测试的同一隔离方式关闭 listener 自动启动后重跑。
...
| 远端同步 | M13-M18 已 push 到 `origin/feat/ai-agent`（最新 `514f301`，本地领先 0 个提交）—— 截至 2026-09-20 快照，实时值见导出页眉 |
| 当前阶段 | **M18 已完成**：T12/T13 前置验证 → V18 轨迹（`TraceRecorder` + 四条链路埋点 + 管理端查询）→ V19 用量（统一埋点 + 成本估算 + 概览接口）→ `AiDegradeGuard` 统一降级 → 前端轨迹页；真实环境端到端验证通过 |
| 最新 Flyway 迁移 | `V19__add_ai_usage_stat.sql`（AI 用量明细；M18 的迁移已到齐） |
| 模块完成度 | **M13-M18 六个模块全部完成**；剩余工作为整理论文/答辩材料与合并回 `main` |
...
| 测试状态 | **416 项：406 通过 + 10 项条件跳过**（跳过项均为需要真实 API Key 的调用测试，含默认关闭的端到端观察类）；前端 **36 项**（**最近一次已验证基线：2026-09-20，收口阶段 4A 全量重跑**） |
...
| 待办 | M18 已全部交付（V18/V19、轨迹与用量接口、统一降级、前端轨迹页）；后续为整理论文/答辩材料与合并回 `main`（推送需单独确认） |
...
| M18 | 可观测性、评估与工程化闭环 | **已完成** | T12/T13 前置验证通过；V18 轨迹 + V19 用量 + 统一降级 + 前端轨迹页全部落地；后端 416 项 / 前端 36 项全绿；真实环境端到端验证（一次对话产生 5 步轨迹，含工具调用）；性能与故障注入两轮观察报告已成文 | `m18-decision-brief.md`、`m18-handoff.md` |
...
| 相关测试 | 75 项全绿 |
| 后端全量 | **407 项：398 通过 + 9 条件跳过，0 失败** |
| 前端 | 33 项通过；`npm run build` 成功 |

---

### M18-4：前端「AI 执行轨迹」页 + 用量概览 + 真实环境端到端验证（2026-09-19）

- **Status:** complete

#### 一、前端（`/admin/ai-traces`）

| 文件 | 内容 |
| --- | --- |
| `pages/admin/AiTraces.jsx` | 一页两块：**用量概览**（近 1/7/30 天的调用数、token、估算成本、降级数；按 Agent / 按天两张表）与**执行轨迹列表**（场景 + 状态筛选、分页、展开看步骤时间线） |
| `api/adminApi.js` | 新增 `pageAiTraces` / `getAiTraceDetail` / `getAiUsageOverview` 三个函数 |
| `App.jsx`、`AdminLayout.jsx` | 新增路由 `/admin/ai-traces` 与「AI 能力」分组下的「执行轨迹」页签 |
| `styles/ai-trace.css` | 页面样式；步骤按类型用左侧色条区分（模型调用 / 工具调用 / 降级 / 路由），扫描时间线时不必逐字读类型名 |

三个刻意的设计：

1. **用量与轨迹同页**：它们回答的是同一问题的两面 —— "这次调用做了什么"与"这些调用花了多少"；
2. **列表不返回 steps，展开才请求详情**：与后端同一条约定，避免列表页响应随轨迹量膨胀；
3. **降级原因直接展示在列表行上**（原因码 + 统一文案），不必展开就能看到"为什么这次没有 AI 结果"。

#### 二、前端验证

| 项 | 结果 |
| --- | --- |
| 新增测试 | `AiTraces.test.jsx` 3 项：用量与列表渲染（含降级原因）、展开后加载步骤时间线、空态提示 |
| 前端测试 | **36 项通过**（原 33 + 新 3） |
| 前端构建 | `npm run build` 成功 |
...
1. `./stop-local.sh` → `./dev.sh` 重启后端（旧进程还是 M17 的代码，不重启就没有新接口）；
2. 探活：`/api/admin/ai/traces` 与 `/api/admin/ai/usage/overview` 未登录均返回 **401**（AdminInterceptor 生效）；
3. 用评测账号 `m17eval_user_01` 登录 → 新建会话 → 提一个**必须调工具**的问题（"站内有哪些关于 Redis 的文章"）；
4. 查库核对产物。

结果（真实运行，非测试）：

```text
ai_execution_trace：1 条 CHAT 轨迹
  step_count=5, total_tokens=7085, status=SUCCESS
  steps 含 RETRIEVE / LLM_CALL / TOOL_CALL，且出现工具名 searchArticles

ai_usage_stat：1 条明细
  QA | prompt=6487 completion=598 total=7085 | result=SUCCESS
  估算成本 0.017758 元（按输入 ¥2 / 输出 ¥8 每百万 token）
```

**这一步的意义**：T12 的结论（工具链只能靠包装 `ToolCallback` 采集）在生产链路得到复验 ——
如果当初把埋点写在"解析最终响应"上，这里会看到一条 5 步里**没有 TOOL_CALL** 的轨迹，
而且不会有任何报错。

---

### M18-5：TokenBudgetGuard（克制版）+ 最终收口（2026-09-19）

- **Status:** complete

#### 一、范围：只做三件事，明确不做四件事

M18 原本把 `TokenBudgetGuard` 标为「时间充足才做」。收尾时按**克制版本**补上：

| 做 | 说明 |
| --- | --- |
| 单用户每日预算 | token 与费用（估算）两个维度，任一超限即拦截 |
| 超预算后统一降级 | 复用 `TraceDegradeReason.BUDGET_EXCEEDED` 的固定文案；对话给兜底回答，推荐退化为「无理由列表」 |
| 单次请求保护 | 输入长度上限（4000 字符）+ 对话输出上限（1024 token） |

| 不做 | 理由 |
| --- | --- |
| 套餐 / 充值 / 余额 | 与本项目「毕业设计 + 工程实践」的定位无关，也没有支付域可依托 |
| 不同会员等级 | 需要产品与运营设计，拍脑袋定档位只会变成不可解释的规则 |
| 分布式配额中心 | 单实例单体应用，引入它等于为不存在的问题增加不可解释的状态 |
| 复杂动态配置后台 | 预算参数走 `application.yml` + 环境变量即可，做后台属于过度设计 |

#### 二、实现：不新增表、不新增迁移

- 新增 `ai/usage/AiTokenBudgetGuard`：判定所需的"该用户今天用了多少"直接聚合
  `ai_usage_stat`（M18 已建 `idx_aius_user_date` 索引），**没有 V20 迁移**；
- 为此在 `AiUsageStatMapper` 增加一条 `selectUserDaily(userId, statDate)` 聚合查询；
- 接入两条**用户主动消耗额度**的链路：
  - `AgentOrchestrator.chat`：在任何模型调用之前判定；命中则走 `persistAndRespond(降级回答)`，
    与正常回答共用同一套「落库 + 轨迹收尾」逻辑（为此把尾部逻辑抽成私有方法，
    避免"被限流的那轮"变成历史里查不到、轨迹停在 RUNNING 的孤儿记录）；
  - `RecommendService.generateReasons`：命中则返回 `ReasonOutcome.degraded(BUDGET_EXCEEDED, ...)`，
    推荐列表与排序**完全不受影响**（复用 M17 就设计好的"无理由推荐"降级路径）；
- **刻意不设闸门**：内容审核（MQ 异步）与运营洞察（管理员触发）——
  让"作者超额"导致内容不被审核，是拿治理正确性换成本；
- `QaAgent` 增加 `maxTokens`（配置 `bitforum.ai.budget.max-output-tokens`，默认 1024）；
- 管理端用量概览新增 `budget` 字段（上限、输入输出保护、生效范围说明），
  让"某个用户为什么被限流"在同一个页面上能解释清楚。

配置（`application.yml` 的 `bitforum.ai.budget`）：`enabled` / `daily-token-limit`（200000）/
`daily-cost-limit`（2.0 元）/ `max-input-chars`（4000）/ `max-output-tokens`（1024），
全部可用环境变量覆盖，`<= 0` 表示不限制。

#### 三、两个**有意的**软边界（写进注释与文档，不假装是硬上限）

1. 判定基于"今天**已落库**的用量"，而本次调用的 token 要等调用结束才写入，
   因此实际用量可能略微超出上限（soft limit）。要做硬上限需要预扣额度，属于"配额中心"范畴，本轮不做。
2. 预算查询失败时**放行**（fail-open）：预算组件自身故障不应让所有人用不了 AI ——
   与「AI 不可用不能影响论坛主流程」是同一条原则。

#### 四、验证

| 项 | 结果 |
| --- | --- |
| 新增测试 | `AiTokenBudgetGuardTest` 10 项（放行 / token 超限 / 费用超限 / 关闭 / 匿名 / 不限 / 输入超长 / 边界 / fail-open / 限额暴露） |
| 链路测试 | `AgentOrchestratorTest` +2（超预算与超长输入**都不调用 Agent**，且轨迹为 `DEGRADED` + `BUDGET_EXCEEDED`）；`RecommendServiceTest` +1（超预算时 `reasonAgent` 不被调用、列表照常返回） |
| 接口测试 | `AdminAiUsageControllerTest` 增加预算口径断言 |
| 前端 | 36 项通过；`npm run build` 通过（预算口径随概览接口返回，页面无需改动） |

#### 五、最终收口（本轮一并完成）

1. **文档统一**：`README.md`（Flyway V1-V19、Redis Stack、AI 技术栈与架构图、AI 能力与边界）、
   `external-ai-briefing.md`（进度/规模/Flyway/已实现能力/踩坑/下一步全部同步）、
   `毕业设计文档总览.md`、`毕业设计进度.md`、`BitForum项目完整历史总结.md`（新增第 16 节）、
   `task_plan.md`（Phase 6 收尾）、`m18-handoff.md`、`m18-status-summary.md`；
2. **未提交改动检查**：提交前逐项确认，不使用 `git add .`；
3. **全量测试**：后端 `mvn test` + 前端 `npm test` / `npm run build`；
4. **push 检查点**：按项目作者的明确指示推送 `feat/ai-agent`（本轮之前 16 个提交都只在本地）；
5. **合并回 `main`**：给出时机建议，不擅自合并。
