...
> 状态：进行中。此节只覆盖交接任务书要求的文档一致性、性能观察和故障注入；不新增业务功能、迁移或重型依赖。

1. **只读基线与文档审计** → 核验 Git、运行依赖、现有测试与动态/历史快照，并列出不一致项。
2. **状态文档修正与导出防冲突** → 验证：重新导出的上下文包将当前状态与历史快照明确分隔。
3. **确定性本地性能观察** → 验证：重复运行不调用真实模型的度量测试，保留脱敏原始样本并计算统计值。
4. **故障注入与预算边界** → 验证：自动化证明运行时模型故障和预算拦截均走统一降级，且不继续调用模型。
5. **最终回归与报告** → 验证：相关测试、构建、Compose 配置、Markdown 链接和 `git diff --check`；无法执行的真实 API 或环境依赖如实写入报告。

**最小实现原则：**先复用现有 `AiDegradeGuard`、`AiTokenBudgetGuard`、`TraceRecorder` 和测试基础设施；仅当现有证据无法满足任务书验收时，才增加最小测试或测试专用观测代码。
...
- [x] 新增 Flyway V14 与分块 / 索引服务
- [x] 接入 RabbitMQ 异步索引队列
- [x] 实现 `RagService` 与引用回答
- [x] 知识库统计接口与管理页

**M15 已完成**（2026-09-19）。向量检索实测与嵌入模型选型见 `findings.md` 6.8 / 6.9，异步索引见 6.10。

### Phase 4：M16 内容审核 Agent

- [x] 新增 Flyway V15 与审核 Agent
- [x] 联动 M2 审核流与人工反馈闭环
- [x] 评测集与对照实验（**实际口径**：开发集 70 条 + 独立测试集 30 条；规范见 `m16-eval-protocol.md`）

**M16 已完成**（2026-09-19）。评测结果：漏放率 0%、误伤率 0%、安全召回率 100%；
自动放行阈值在测试集上覆盖率 0%，故**保持默认关闭**（依据见 `progress.md` M16 小节）。
