...
- 下一步：**等作者决策**（均需明确授权）：
  ① 按建议的 5 个提交拆分提交这批收口改动；
  ② push `feat/ai-agent` 并等**新 HEAD 的远端 CI** 结果（旧 HEAD 的 CI 不能覆盖未提交改动）；
  ③ 合并回 `main`（建议 `merge --no-ff` + 合并后 CI + 打 tag）；
  ④ 是否补两项建议的补强（主流程隔离观察、审核/洞察链路级故障注入）——**不补则必须保持"未验证"口径**
  **在得到授权前不要 commit / push / merge / tag**
...
- 上一步停在哪里：**阶段 4B 完成 + 阶段 4C（外部复查整改）完成**；未 commit / push / merge / tag。
- 恢复后先检查：
